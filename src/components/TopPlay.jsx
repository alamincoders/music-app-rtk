...
const TopChartCard = ({ song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => (
  <div className="w-full flex flex-row items-center hover:bg-[#4c426c] py-2 p-4 rounded-lg cursor-pointer mb-7">
    ...
    <div className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col" ref={divRef}>
      ...
      <div className="w-full flex flex-col mt-8">
        ...
        <Swiper slidesPerView="auto" spaceBetween={15} freeMode centeredSlides centeredSlidesBounds modules={{ FreeMode }} className="mt-4">
          {topPlays?.map((song, i) => (
            <SwiperSlide key={i} style={{ width: "25%", height: "auto" }} className="shadow-lg rounded-full animate-slideright">
              <Link to={`/artists/${song?.artists[0]?.adamid}`} className="">
                <img src={song?.images?.background} alt="background" className="rounded-full w-full object-cover" />
              </Link>
            </SwiperSlide>
          ))}
        </Swiper>
